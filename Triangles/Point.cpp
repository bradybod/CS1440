...
bool Point::isEquivalentTo(const Point &otherPoint) const
{
    ...
    return edge.getLength() < m_minDistance; //Error return edge.getLength() > m_minDistance
}
...
void Point::initialize(const std::string &pointStr)
{
    ...
    if (split(pointStr, ',', values, 3))
    {
        ...
        m_z = convertStringToDouble(values[2], &m_valid); //BUG m_y changed to m_z
        if(m_valid){
            checkForInfinity();
        }
    }
...
void Point::checkForInfinity()//Error found in m_z == INFINITY
{
